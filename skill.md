...
`search` runs a one-shot query (`--patterns` clusters matches into drain-style templates with counts — the fastest firehose summary). `tail` polls and follows new events (severity is auto-colored: errors red, warns yellow). `traces` has subcommands (`list`, `show`, `services`, `operations`, `aggregate`, `shape`) over the `.traces` logset. `send` posts one event (`-m`) or streams NDJSON/text lines from stdin. `fields` discovers top-level keys in a dataset. `context` shows events around a specific anchor event.
