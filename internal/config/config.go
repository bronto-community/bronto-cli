...
var envKeys = map[string]string{
	...
	"device_auth_url":  "BRONTO_DEVICE_AUTH_URL",
	"device_token_url": "BRONTO_DEVICE_TOKEN_URL",
	"device_client_id": "BRONTO_DEVICE_CLIENT_ID",
}
...
// from files. device_* configure the experimental RFC 8628 login
// endpoints for `bronto auth login --device`.
var userFileKeys = []string{"profile", "region", "base_url", "output", "default_dataset", "timeout", "max_retries", "ingest_url", "device_auth_url", "device_token_url", "device_client_id"}
...
// audit, HIGH. device_auth_url/device_token_url are likewise excluded: a
// project file must not redirect where a browser login mints and sends
// its token.
